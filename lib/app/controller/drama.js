...
class DramaCtrl {
  ...
  async getDramaList(req, res) {
    try {
      let dramaList = await dramaModel.find(
        {
          airTime: {
            "$elemMatch": {
              "$gt": new Date(req.query.airtimeStart),
              "$lt": new Date(req.query.airtimeEnd)
            }
          }
        }
      ).exec();
      ...
      logger.error(e.message);
      respondError(req, ress, codes.DramaListGetFailed.code);
    }
  }

  async getDramaChatroom(req, res) {
    try {
      let getDramaChatroom = await dramaModel.findOne({ _id: req.params.did }).exec();

      if (null === getDramaChatroom) {
        respondError(req, res, codes.GetDramaChatroomFailed.code);
      } else {
        respondSuccess(req, res, getDramaChatroom.chatrooms);
      }
