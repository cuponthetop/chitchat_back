...
async function loadUserOnRequest(req, res, param) {
  try {
    let user = await userModel.findOne(param).exec();

    if (null === user) {
      return respondError(req, res, codes.UserNotFound.code);
    } else {
      req.info = req.info || {};
      req.info.user = user;
      return null;
...
class UserCtrl {
  ...
  async addLikedDrama(req, res) {
    try {
      ...
      if (user !== null) {
        if (!sanitizeObjectId(req.body.did)) {
          return respondError(req, res, codes.InvalidObjectID.code);
        }

        user.likedDrama.push(new ObjectId(req.body.did));

        let index = user.likedDrama.findIndex((p) => p.toString === req.body.did);
      ...
      logger.error(e.message);
      return respondError(req, res, codes.AddLikedDramaFailed.code);
    }
  }
  ...
  async removeLikedDrama(req, res) {
    try {
      ...
      logger.error(e.message);
      return respondError(req, res, codes.RemoveLikedDramaFailed.code);
    }
  ...
  async getLikedDrama(req, res) {
    try {
      ...
      if (user !== null) {
        let likedDrama = await dramaModel.find({
          _id: {
            $in: user.likedDrama.map(function(o){ return mongoose.Types.ObjectId(o); })
          }
        })
          .populate({ path: 'chatrooms', model: 'chatrooms' })
          .exec();

        if (likedDrama === null) {
          respondError(req, res, codes.getLikedDramaFailed.code);
        }
        else {
      ...
      logger.error(e.message);
      return respondError(req, res, codes.GetLikedDramaFailed.code);
    }
  }
}
