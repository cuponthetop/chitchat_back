...
class UserCtrl {
  ...
  async createUser(req, res) {
    try {
      ...
      let email = await userModel.findOne({ email: newUser.email }).exec();
  ...
  async updateUser(req, res) {
    if (req.info.user !== undefined) {
      // TODO:
      try {
        let user = await userModel.find({ _id: req.params.uid }).exec();
        ...
        if (null === user) {
          ...
          let savedUser = await user.save();
          respondSuccess(req, res, savedUser.nickname);
        }
