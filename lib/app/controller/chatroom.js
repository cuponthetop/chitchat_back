...
import { default as chatroomModel } from '../model/chatroom';
...
class ChatroomCtrl {
  ...
  async getPopularDramaList(req, res, next) {
    try {
      ...
      let dramas = await dramaModel
        .find({
          _id: {
            $in: popularDramaList.map(function (o) { return o._id; })
          }
        })
        .populate({
          path: 'chatrooms',
